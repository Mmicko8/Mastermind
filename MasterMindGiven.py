...
def draw_board(root):
    ...
    canvas.create_window(180, MAX_NUMBER_OF_MOVES*40+90, anchor=NW, window=sublist_button)
    ...
    canvas.create_window(50, MAX_NUMBER_OF_MOVES*40+130, anchor=NW, window=hint_button)

    # Draw the quit button
    quit_button = Button(canvas, text="Quit", command=root.destroy)
    quit_button.configure(width=10, relief=FLAT)
    canvas.create_window(180, MAX_NUMBER_OF_MOVES*40+130, anchor=NW, window=quit_button)
...
def color(color_name):
    ...
    if len(guess_combination) == NUMBER_OF_CIRCLES:
        ...
        canvas.itemconfig(ovals[current_move][len(guess_combination)],fill= color_name)
